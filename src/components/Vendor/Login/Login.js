...
export default function VendorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:8000/vendors/auth/login", {
      ...
      const stallId = res.data.stall_id;
      const vendorName = res.data.vendor_name;
      const vendorPhone = res.data.vendor_phone;

      loginUser(token, vendorId, "vendor", vendorPhone, stallId, vendorName);
      navigate("/items-vendor");
    } catch (err) {
