export const getUserDetails = async (userId, token) => {
  const res = await fetch(`http://localhost:8000/admin/${userId}`, {
...
export const fetchBuildings = async (adminId, token) => {
  const res = await fetch(`http://localhost:8000/buildings/buildings/by-admin/${adminId}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const createManager = async (data, token) => {
  try {
    const res = await fetch('http://localhost:8000/managers/', {
      method: 'POST',
      headers: {
...
export const fetchManagersByBuilding = async (buildingId, token) => {
  const res = await fetch(`http://localhost:8000/managers/building/${buildingId}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const deleteManager = async (managerId, token) => {
  const res = await fetch(`http://localhost:8000/managers/${managerId}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${token}` }
  });
